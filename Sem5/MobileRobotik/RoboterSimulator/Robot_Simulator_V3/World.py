...
from RoboterSimulator.Robot_Simulator_V3 import KeyboardController
from RoboterSimulator.Robot_Simulator_V3 import OccupancyGrid
from RoboterSimulator.Robot_Simulator_V3 import geometry
...
class World:
    ...
    def __init__(self, width, height):
        ...
        self._win = GraphWin("Staubsaugsimulator 3000", int(1000.0 * width / height), 1000, autoflush=False)
    ...
    #
    def drawPolylines(self, polylines, color='yellow'):
        self.undrawLines()
    ...
    def setRobot(self, robot, pose):
        ...
        # Update status bar
        self._clockTimeText.setText(
            "Clock Time: %4.2f Driven Distance: %4.2f Position: %4.2f, %4.2f, %4.2f v, omega: %4.2f, %4.2f Picked Up: %r"
    ...
    def removeRobot(self, robot):
        for line in self._sensorLines:
            line.undraw()
        self._robot = None
        self._robotCircle.undraw()
        self._robotCircle = None
        self._robotTheta = None
        self._robotLine.undraw()
        self._robotLine = None
        robot.removeWorld()
        self._updateWindow()

    def resetRobot(self, robot, pose):
        self.removeRobot(robot)
        self.setRobot(robot, pose)
    """
    ...
    def _senseBox(self):
        if not self._boxSensor:
            return None
        if not self._boxesSensedDistAngle:
            p = self._robotCircle.getCenter()
